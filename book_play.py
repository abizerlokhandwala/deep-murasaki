...
from book_train import make_model
from book_parser import bb2array, attacks, numeric_notation
...
MODEL_DATA = 'conv3x64_4096_2048_1024_2017-07-26_195636.model'

class Murasaki(Player):
	def __init__(self):
		self._model, _ = make_model()
		self._model.compile(loss='mean_squared_error', optimizer='adadelta')
		self._model.load_weights( MODEL_DATA )
...
#		X = numpy.array([bb2array( gn_current.board(), flip=(color==1) )])
		...
		for m in moves :
			board.push( m )
			X.append( numpy.array( [bb2array(board), attacks( board, chess.WHITE), -attacks( board, chess.BLACK)]) )
			board.pop()

		predicted = sorted( zip( [i[0] for i in self._model.predict( numpy.array(X))], moves ), reverse=True)
		print predicted

		best_value, best_move = predicted[0]
